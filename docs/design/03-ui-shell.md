...
**Hareket (`Theme+Motion.swift`).** Süreler `quick 0.12` / `standard 0.2` / `panel 0.3` / `pulse 1` + hazır `Animation` sabitleri (`quickEase`, `standardEase`, `standardOut`, `panelEase`, `statusPulse`). Gecikmeler ayrı `Duration` sabitleridir: `hoverRevealDelay 500ms`, `hoverOpenDelay 350ms`, `hoverCloseDelay 200ms`, `sidebarRevealDelay 100ms`, `searchDebounce 150ms`. Terminal kartı header'ında Projects ajan satırıyla aynı `AgentActivityIcon(AgentActivityState)` kullanılır; hemen sağında `TerminalIdentityIcon` durur (karar 45/81): `TerminalMeta.provider` Claude/Codex ise bundle logosu (`ProviderIcon`), `nil` (düz shell) ise `terminal` SF Symbol'ü — "ne durumda" ve "kim" iki ayrı gliftir. `Theme.statusColor` yalnız kompakt `TerminalChipStrip` noktalarının ortak paletidir.
...
Karar 49/72/82: `ProjectsPanel` üç katmandır — uygulama-içi özel payload ile sürüklenerek sıralanabilen proje satırı, `CheckoutRow` (`.original(Repo)` için tekrarlanan proje adı/rozet yerine varsa gerçek branch, yoksa `main`; `.workspace(ProjectWorkspace)` için ad + branch) ve altında `AgentRow` listesi. Proje sırası `sidebarProjectPaths`'in sırasıdır; drag sonucu aynı alana yazılır ve böylece ⌃1–⌃9 sırası panelle aynı kalır. Ajan modeli `TerminalListStore.terminals(in:)`'den türer; glif `AgentActivityIcon(AgentActivityState)` ve kimlik `TerminalIdentityIcon` ayrı çizilir, zaman `RelativeTimeFormatter.shortLabel(meta.lastActivityAt)`. Birden fazla ajan `N agents` toggle'ı ile daraltılır (view-lokal state). Proje aksiyonları sabit ölçülü alanda hover/klavye odağında görünür ve sağ tık menüsü de korunur; eylemler `ShellContext.focusAgent` / `requestDeleteWorkspace` / `confirmDeleteWorkspace(force:)` / `cancelDeleteWorkspace` / `forgetWorkspace` koordinasyonlarına ve `ShellActions.revealPath`e gider. `focusAgent`, checkout grid modundaysa yalnız restore+focus yapar; checkout'ta zaten maximize edilmiş bir terminal varsa seçilen ajanı o solo yüzeyin yeni hedefi yapar. `DeleteWorkspaceDialogOverlay` (`OverlayID.deleteWorkspaceDialog`) `ModalOverlay + Panel(.modal)` ile çizilir; store'daki `deleteError` ve `canForceDelete` butonu `Force Delete`e çevirir. Oluşturma satırı `WorkspaceOperationRow`a ayrıldı.
