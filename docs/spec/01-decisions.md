...
### 21. Spawn başlangıç komutu quiescence-gate'li (2026-08-11, onaylı bug düzeltmesi)
Electron paritesindeki "spawn + anında `write(command)`" davranışı bir yarış içeriyordu: shell startup'ında stdin okuyan interaktif sorular (oh-my-zsh auto-update `[Y/n]` tek-tuş okuması vb.) enjekte edilen komutun ilk karakterlerini yutuyor, `claude --session-id ...` → `laude ...` olarak çalışıp `command not found` üretiyordu. Kullanıcı onayıyla parite bilinçli kırıldı: başlangıç komutu `LaunchCommandGate`'te bekletilir, Spec 4'ün çıktı-sessizliği tetiğinde ekranın alt satırlarına bakılarak shell hazırsa (ekran dolu + son dolu satır tek-tuş onay sorusu değil) **bir kez** yazılır; soru göründüğü sürece beklenir (kullanıcı cevaplar, sonraki sessizlikte enjekte edilir). Normal açılışta maliyet ~350 ms gecikmedir. Kapsam: yalnız `spawn(command:)` yolu — persona/ActionEngine spawn-sonrası yazımlarındaki aynı yarış ayrı iştir. Detay: [10-main-terminal-pty.md](./10-main-terminal-pty.md) §2.
